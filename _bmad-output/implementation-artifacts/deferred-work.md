...
- **W2 (LOW): `Path.GetExtension` clasifica como asset slugs con punto** — Una ruta futura tipo `/noticias/fibra-sube-2.5` esquivaría el middleware sin aviso. Anotar como restricción de diseño en la historia 11-4 (slugs de noticias): los slugs no deben contener puntos, o cambiar el filtro a lista de extensiones conocidas.
- **W3 (LOW): Variantes de URL no normalizadas sirven el template genérico sin canonical** — `//calculadora`, `/calculadora%2F`, punto final no matchean el diccionario y caen al fallback con el title genérico. Contenido duplicado de bajo riesgo; el sitemap/robots de 11-3 mitiga. Considerar colapsar `//` en la normalización del provider.
- **W4 (LOW): JSON-LD FAQPage con pregunta no visible en la página** — La tercera pregunta ("¿Qué es el CBFI de una FIBRA?") no tiene Q&A visible en `CalculadoraPage.tsx`; Google exige contenido visible para rich results FAQ. Alinear contenido visible o recortar el JSON-LD cuando 10-2/calculadora interactiva toque la página.
- **W5 (LOW): `GET /index.html` elude el middleware** — Por tener extensión `.html` pasa a StaticFiles y sirve el template crudo (comentario sin reemplazar, sin canonical). Duplicado menor de la home; mitigable con 301 `/index.html` → `/`.
- **W6 (MEDIUM): `/calculadora` huérfana — sin enlaces internos** — Decisión del usuario: se creará una funcionalidad posterior que la enlazará; mientras tanto el sitemap de 11-3 da descubribilidad a crawlers. Verificar al implementar esa funcionalidad que el enlace exista (la orfandad es el patrón que produce "Crawled - currently not indexed").
- **W7 (LOW): Sweep de metadata client-side en páginas cubiertas por el middleware** — Tras la decisión "el servidor es la única fuente de verdad", quedan title/meta client-side duplicados (sin canonical conflictivo) en HomePage, CalendarioPage, ComparadorPage, CatalogoPage, NoticiasListPage, ConoceLasFibrasPage y FundamentalesPage; además sus textos difieren de los del SpaMetadataProvider. Retirarlos (o alinear textos) en una pasada dedicada. Las rutas dinámicas (`/fibras/:ticker`, `/noticias/:id`) NO se tocan: su meta client-side es la única fuente.
- **W8 (LOW): Test flaky `AiModeGetPutTests.GetOpsNewsList_WithSearch_ReturnsOnlyMatchingArticles`** — Falló una vez en suite completa y pasó aislado y en re-run completo (272/272). Probable fuga de aislamiento de datos entre tests de la colección. Sin relación con 11-2; investigar si reincide.
...
## Deferred from: code review of 11-3-sitemap-xml-y-robots-txt (2026-06-11)

- **D1 — Query a BD por cada GET/HEAD de `/fibras/*` y `/sitemap.xml` sin caché** [`src/Server/Api/Middleware/FibraSlugRedirectMiddleware.cs:61`, `SeoEndpoints.cs:32`] — Cada page view de ficha y cada hit de crawler paga scope DI + roundtrip a BD para un mapa ticker→slug que cambia raramente. Ya previsto en Dev Notes de 11.3: agregar `IMemoryCache` (expiry ~1h) si hay presión de performance.
- **D2 — Prerender legacy: drift `FIBRAS_SEED`↔BD, 301 append-slash de `UseDefaultFiles` y fichas sin metadata server-side** [`src/Web/Main/scripts/prerender.mjs:22`] — El deploy usa `npm run build` (sin prerender), así que no afecta producción hoy. Si algún día se usa `build:full`: los directorios `dist/fibras/{slug}/` provocan que `UseDefaultFiles` (RedirectToAppendTrailingSlash) emita 301 a la versión con slash — contradiciendo sitemap y canonical — y el seed estático puede divergir del FullName en BD. Además `SpaMetadataProvider` no cubre `/fibras/*`: las fichas sirven HTML genérico server-side (pre-existente). Candidato a historia: metadata server-side para fichas de FIBRA.
- **D3 — Triple implementación del slugify (C#/TS/mjs)** [`src/Web/Main/scripts/prerender-utils.mjs:1`] — La paridad se sostiene solo con tablas de ejemplos hardcodeados en tres suites. Consolidar mjs importando el `.ts` (node ya corre `--experimental-strip-types` en el script `test`) cuando se decida el destino del prerender legacy.

## Deferred from: code review of 11-4-slug-urls-noticias (2026-06-11)

- **D1 — GUID legacy responde 200+canonical en vez de 301 server-side** [`src/Server/Api/Middleware/NewsMetadataMiddleware.cs`] — CA-5 prescribe redirect client-side (`useEffect` + `navigate replace`), invisible para crawlers sin JS: dos URLs con el mismo contenido en 200. El canonical mitiga pero no consolida señales como el 301 que ya estandarizó `FibraSlugRedirectMiddleware` para fibras. Candidato: 301 server-side `/noticias/{guid}` → `/noticias/{slug}` en historia futura.
- **D2 — `BASE_URL` hardcodeado client-side en NoticiaPage** [`src/Web/Main/src/modules/noticia/NoticiaPage.tsx:12`] — Mismo patrón pre-existente que `FibraPage.tsx:165` (aceptado en 11.3): en staging el cliente inyecta canonical hacia producción. Centralizar dominio en config/env del frontend cuando se toque el módulo.
- **D3 — Sin caché de `index.html` ni del lookup BD por request en `/noticias/*`** [`src/Server/Api/Middleware/NewsMetadataMiddleware.cs:117`] — Consistente con `SpaMetadataMiddleware` (11.2) y con D1 de 11.3. Si hay presión de performance, `IMemoryCache` para HTML + artículo.
- **D4 — Tests InMemory no ejercitan índice único filtrado, colación CI de SQL Server ni `ExecuteUpdateAsync`** [`tests/Unit/Infrastructure.Tests/Persistence/Repositories/NewsRepositorySlugTests.cs`] — Las ramas de colisión/case-sensitivity de slug pasan en verde sin validar comportamiento de producción. Limitación documentada del toolchain; cubrir con tests de integración contra SQL Server real si se monta esa infraestructura.
- **D5 — `GetBySlugAsync` no filtra `Status`: artículos Pending/Failed accesibles por URL adivinable** [`src/Server/Infrastructure/Persistence/Repositories/News/NewsRepository.cs`] — Paridad con `GetByIdAsync` pre-existente (`FindAsync` sin filtro de Status ni DeletedAt). Decidir a nivel producto si el detalle público debe exigir `Status == Processed` (aplicaría a ambos endpoints).
- **D6 — Metas duplicadas tras hidratación (bloque SSR estático + tags de React 19)** [`src/Web/Main/src/modules/noticia/NoticiaPage.tsx`] — React 19 no dedupe contra los tags inyectados por el middleware; scrapers que ejecutan JS ven `og:*`/canonical duplicados. Inherente al enfoque 11.x en todas las páginas públicas; mitigado al alinear contenidos (P3/P4 de este review).

## Deferred from: code review of 11-6-herramientas-hub-privado (2026-06-12)

- **D1 (LOW): Link Oportunidades no deep-linkea al tab Promediar** — El hub apunta a `/oportunidades` (correcto per spec), pero el tab activo usa `useState` en lugar de URL params; el usuario aterriza en el tab por defecto. Requiere cambio arquitectónico en OportunidadesPage para que el tab sea URL-driven.
- **D2 (LOW): Token Banxico no usa secrets store** — `Banxico:Token` en `appsettings.json` tiene valor vacío por diseño; para producción debe inyectarse vía env var (`Banxico__Token`) o secrets manager. Preocupación de infraestructura/deploy, fuera del scope del story.
- **D3 (LOW): `DateTimeOffset.UtcNow` en job en vez de fecha publicada por Banxico** — El campo `Cetes28dRateUpdatedAt` refleja cuándo el job persistió el dato, no cuándo Banxico publicó la tasa. La fecha de Banxico está disponible en `datos[0].fecha` (formato DD/MM/YYYY). Cambio de semántica del campo; el spec define `UtcNow` explícitamente.
- **D4 (LOW): Sin output cache en endpoint `/api/v1/market/indicadores`** — El dato cambia como máximo una vez por semana (miércoles). Agregar `OutputCache` con TTL de 1h cuando se toque el endpoint en historia futura.
- **D5 (LOW): Sin estado de error en UI cuando `fetchIndicadores` falla** — La API de indicadores puede fallar por red o 500; el campo CETES queda vacío sin informar al usuario. El campo vacío cumple AC5 (placeholder "ej. 9.50"), pero no distingue "rate no disponible" de "endpoint caído". Mejora UX diferida.
- **D6 (LOW): ISR factors hardcoded sin disclaimer fuera del tooltip** — Los factores 70% (FIBRA) y 80% (CETES) son definidos explícitamente por el spec como aproximaciones. El tooltip los menciona; no hay disclaimer adicional en la UI. Evaluar si el equipo legal/producto requiere texto explícito de "estimación ilustrativa".
- **D7 (LOW): `IsRelational()` en `UpdateCetesRateAsync` — patrón pre-existente en el codebase** — El repositorio bifurca comportamiento relacional vs. InMemory usando `IsRelational()` en código de producción. Misma deuda que otros repos del proyecto. Refactorizar junto con la limpieza de la capa de repositorios.
- **D8 (LOW): Sin try/catch en `IndicadoresEndpoints` ante BD unavailable** — La excepción propagaría como 500 sin body problem-details. El middleware global de manejo de errores del framework debería capturarla, pero no garantiza el formato problem-details en todos los casos.
