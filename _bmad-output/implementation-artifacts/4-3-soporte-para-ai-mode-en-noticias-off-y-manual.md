...
### Review Findings — Hardening Pass (2026-05-21)

#### Action Items

- [x] [Review][Patch] P1: `clearOpsAccessToken` se llama incondicionalmente antes de verificar si el error es 401 — errores de red (timeout, 500) borran el token válido y fuerzan re-login innecesario [`src/Web/Ops/src/api/authApi.ts:refreshOpsSession`] — fix: mover `clearOpsAccessToken()` dentro del bloque `if (error.status === 401)` y solo llamarla en ese caso
- [x] [Review][Patch] P2: `assertOpsAccessToken` lanza `Error` sin despachar `OPS_AUTH_REQUIRED_EVENT` — si el token desaparece entre el gate check y la ejecución del query, el usuario ve el mensaje de error en la UI en vez de ser redirigido al login [`src/Web/Ops/src/api/opsAuth.ts:assertOpsAccessToken`] — fix: llamar `notifyOpsAuthRequired()` antes del `throw`
- [x] [Review][Patch] P3: `blocklistQuery` (definida en `App`) queda en estado error tras bootstrap exitoso porque no se invalidan las queries al completar el refresh — el usuario autenticado por cookie ve el error de auth en la tabla de blocklist hasta que refresca la página [`src/Web/Ops/src/components/OpsLoginGate.tsx:bootstrapSession`] — fix: llamar `void queryClient.invalidateQueries()` después de `setAuthStatus('authenticated')` en el path de bootstrap exitoso
- [x] [Review][Patch] P4: Test e2e `'Ops pide login cuando no hay sesión'` está dentro del `describe` que tiene `beforeEach` con `seedOpsAuth` — el test pasa correctamente pero por efecto implícito (el `clearOpsAccessToken` dentro de `refreshOpsSession` borra el token sembrado); si se modifica `refreshOpsSession` para no limpiar en 401, el test dejaría de validar el escenario real [`src/Web/Main/tests/e2e/news-epic4.spec.ts`] — fix: extraer el test a su propio `describe` sin el `beforeEach` de auth, o añadir `clearStorage` explícito al inicio del test

#### Deferred

- [x] [Review][Defer] D1: Token de AdminOps en `sessionStorage` expuesto a XSS [`src/Web/Ops/src/api/opsAuth.ts`] — deferred, decisión arquitectural aceptada para SPA; el Ops SPA tiene surface de ataque reducida (acceso restringido a AdminOps)
- [x] [Review][Defer] D2: `getStoredOpsAccessToken` lee de `localStorage` sin que `storeOpsAccessToken` escriba ahí [`src/Web/Ops/src/api/opsAuth.ts`] — deferred, compatibilidad intencional con versiones anteriores que podían haber usado localStorage
- [x] [Review][Defer] D3: Bootstrap `catch` reutiliza token previo como indicador de sesión válida cuando `refreshOpsSession` lanza excepción de red [`src/Web/Ops/src/components/OpsLoginGate.tsx:bootstrapSession`] — deferred, el primer 401 del backend recupera el estado vía `OPS_AUTH_REQUIRED_EVENT`
- [x] [Review][Defer] D4: Multi-tab: `clearOpsAccessToken` en Tab A no propaga estado React en Tab B [`src/Web/Ops/src/api/opsAuth.ts`] — deferred, limitación conocida de auth por storage; tool de uso single-admin en MVP
- [x] [Review][Defer] D5: Estado `'checking'` sin timeout — si el backend no responde, la pantalla "Restaurando sesión" persiste indefinidamente [`src/Web/Ops/src/api/authApi.ts:refreshOpsSession`] — deferred, timeout del browser como fallback; aceptable para tool admin de bajo volumen
...
- 2026-05-21: Addressed code review findings Hardening Pass (P1-P4) — `clearOpsAccessToken` solo se llama en 401 (no en errores de red); `assertOpsAccessToken` despacha `OPS_AUTH_REQUIRED_EVENT` antes de lanzar; bootstrap exitoso invalida queries para recuperar `blocklistQuery` de estado error; test e2e extraído a `describe` propio sin contaminación de `beforeEach` de auth. Build Ops OK.
